...
export class ProgressionController {
  ...
  @Get('me')
  @ApiOperation({
    summary: 'Giriş yapan kullanıcının XP ve level durumunu getirir',
  })
  @ApiOkResponse({
    description: 'XP ve level bilgileri başarıyla getirildi',
  })
  @ApiUnauthorizedResponse({
  ...
  getProgress(@Req() request: AuthenticatedRequest) {
    return this.progressionService.getProgress(request.user.sub);
  }

  @Post('test-xp')
  @ApiOperation({
    summary: 'Geliştirme ortamında kullanıcıya test XP’si ekler',
  })
  @ApiOkResponse({
  ...
  addTestXp(@Req() request: AuthenticatedRequest, @Body() addXpDto: AddXpDto) {
    ...
    return this.progressionService.addXp(request.user.sub, addXpDto.amount);
  }
}
